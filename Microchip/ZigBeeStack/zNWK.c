...
#include "Console.h"
#include "zPHY_MRF24J40.h"//Son53 them de thiet lap cong suat tu lop mang
...
//DKCS: Dinh dang 2 goi RouteRequest va RouteReply
typedef struct _ROUTE_REPLY_COMMAND
{
    ...
    BYTE            indexPt;    //Luu tru cong suat phat ben goi Reply
    BYTE            indexPtlink;//Luu tru Pt mong muon tinh duoc trong qua trinh Request
    BYTE            metric[4];  //Ca ben Rquest va ben Reply deu chua thong so nay
#endif
} ROUTE_REPLY_COMMAND;
...
typedef struct _ROUTE_REQUEST_COMMAND
{
    ...
    BYTE            indexPt;//Luu cong suat phat hien tai cua nut can dieu chinh cong suat
    BYTE            metric[4];//khoi tao gia tri ban dau bang 0
#endif
...
BOOL RouteAlongTree( SHORT_ADDR destTarget, SHORT_ADDR *destNextHop );
    ...
    BYTE      PT = PA_LEVEL;//Son53 khoi tao cong suat phat ban dau cho nut mang
    #pragma udata
									...
									if(1)
										{
										    ...
										    requestMetric = unPack(rreq.metric)+getmetrics(params.MCPS_DATA_indication.mpduLinkQuality,_Tpass/Tcharge,indexRequestPt);//Son53
										#if defined(USE_USART)	
											printf("\r\nrequestMetric = " );
...
sendBackRJRsp:						    GetMACAddress(&tempLongAddress);
	                    			 ...
	                    			 if(1)
										{
										    ...
										    PHYSetOutputPower(indexReplyPt);//Son53 thiet lap cong suat phat tren phan cung
										#if defined(USE_USART) 
											printf("\r\nlastMetric =" );
											printfloat(unPack(rrep.metric));
										#endif
										    // Tinh metric
										    //replyMetric = unPack(rrep.metric)+getmetrics(rrep.metric,rrep.indexPt,indexReplyPt, params.MCPS_DATA_indication.mpduLinkQuality, 1);
										    replyMetric = unPack(rrep.metric)+getmetrics(params.MCPS_DATA_indication.mpduLinkQuality,_Tpass/Tcharge ,indexReplyPt);//Son53
										#if defined(USE_USART)	
											printf("\r\nreplyMetric = " );
                        ...
                        else
                        {
							...
							routingStatus = GetRoutingAddress( FALSE, nwkDestinationAddress, nwkFrameControlLSB.bits.routeDiscovery, &macAddress );
                                ...
                                {
                                     case ROUTE_SEND_TO_MAC_ADDRESS:   //@cuong doan code nay goi du lieu da xac
																		// dinh dia chi can gui den dau.
...
void CreateRouteReply( SHORT_ADDR nwkSourceAddress, BYTE rdIndex, ROUTE_REQUEST_COMMAND *rreq )
{
    ...
    TxBuffer[TxData++] = PA_LEVEL;//gan them truong cong suat phat mac dinh vao goi tin request dau tien
...
//	PT = PA_LEVEL; //Son53 dieu chinh lai 
    #endif
    foundNextHop = FALSE;
    pCurrentRoutingEntry = routingTable;
    i = 0;
